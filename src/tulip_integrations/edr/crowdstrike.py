...
def cs_isolate(host_id: str) -> dict[str, Any]:
    ...
    url, token = _base()
    if url and token:
        return _cs_post(url, token, "/devices/entities/devices-actions/v2?action_name=contain", host_id)
...
def _cs_get(url: str, token: str, path: str) -> dict[str, Any]:
    ...
    with httpx.Client(base_url=url, headers={"Authorization": f"Bearer {token}"}, timeout=30.0) as c:
        resp = c.get(path)
...
@tool(name="cs_host_timeline", description="Pull a host's recent EDR timeline from CrowdStrike Falcon")
async def cs_host_tool(host: str, window: str = "24h") -> str:
    return as_json(cs_host_timeline(host, window=window))


@tool(name="cs_detections", description="List open CrowdStrike Falcon detections, optionally for one host")
async def cs_detections_tool(host: str = "") -> str:
    return as_json(cs_detections(host or None))


@tool(name="cs_isolate", description="Network-contain a host via CrowdStrike Falcon (a write)", idempotent=True)
